...
module.exports = class RabbitBroker {
  ...
  _validate(opts) {
    ...
    if (!opts.connectUrl) errors.push('connectUrl not defined')

    if (!opts.callback) errors.push('callback not defined')
  ...
  *consume(callback) {
  ...
  stop() {
    if (this._consumerTag && this._consumerChannel) {
      return this._consumerChannel.cancel(this._consumerTag).then(() => {
        this._consumerTag = null
        this._consumerChannel = null
      })
    }
