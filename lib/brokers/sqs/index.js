...
module.exports = class SqsBroker {
  ...
  _validate(opts) {
    if (opts.validate === false)
      ...
      if (!opts.aws)
        errors.push("worker need aws configs")
      else if (!opts.aws.region)
        errors.push("worker need aws region")

      if (!opts.name)
        errors.push("worker need a name to groupId sqs")

      if (!opts.queue)
        errors.push("worker need a queue target")
  ...
  consume(callback) {
    ...
    this._getQueueUrl().then(QueueUrl => {
      ...
      if (this._consumeLoop) {
        throw new Error(`Worker is already consuming queue from URL ${QueueUrl}`)
      }
