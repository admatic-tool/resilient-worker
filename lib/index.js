...
const WorkerFactory = {
   ...
   *
   * @param {boolean} [opts.reconnectConsumer=false] - Should consumer try to reconnect? (only for rabbit broker)
   * @param {number} [opts.reconnectTimeoutSeconds=1] - Time between consumer reconnection retries (only for rabbit broker)
   * @param {string} [opts.connectUrl] - Connection URL (only for rabbit broker)
   * @param {object} [opts.queueOptions] - Options of the queue to be asserted (only for rabbit broker)
   *
