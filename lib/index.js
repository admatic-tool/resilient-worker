
"use strict"

const uuidv4 = require("uuid/v4")
const { EventEmitter } = require("events")
const co = require("co")

const brokers = {
  rabbit: "./brokers/rabbit/",
  sqs: "./brokers/sqs/",
}


// helpers
const BulkMessages = require("./messages/bulk-messages")

const wait = milisseconds =>
  new Promise(resolve => setTimeout(resolve, milisseconds))

const WorkerFactory = {

  createWorker(config) {

    const { broker = "rabbit" } = config

    const BrokerClass = require(brokers[broker])
    const emitter =  new EventEmitter()
    const _broker = new BrokerClass(config)

    const {
      callback,
      name,
      successCallback,
      failCallback,
      max_try,
      retry_timeout = 0,
      prefetch = 1,
    } = config

    const worker = {

      on(event, callback) {
        emitter.on(event, callback)
      },

      log(level, msg, message, ...resources) {
        emitter.emit(...[ "log", name, level, msg, message, ...resources ])
      },

      startBulk() {
        const { log } = this

        const bulkSize = prefetch
        const messageAccumulator = []

        setInterval(() => {
          if (messageAccumulator.length > 0)
            emitter.emit("bulk:limit", new BulkMessages(messageAccumulator.splice(0, bulkSize)))
        }, 10 * 1000)

        emitter.on("msg:received", msg => {
          if (msg === null) {
            log("debug", {}, "cancelled")
            return
          }

          messageAccumulator.push(msg)

          if (messageAccumulator.length >= bulkSize) {
            emitter.emit("bulk:limit", new BulkMessages(messageAccumulator.splice(0, bulkSize)))
          }
        })

        emitter.on("bulk:limit", messages => {

          try {

            const execution = callback(messages)

            if(execution.then)
              execution.then(() =>
                emitter.emit("bulk:processed", messages)
              ).catch(err => {
                messages.setAllFailed(err)
                emitter.emit("bulk:processed", messages)
              })

          } catch (err) {
            messages.setAllFailed(err)
            emitter.emit("bulk:processed", messages)
          }
        })

        emitter.on("bulk:processed", messages => {
          const tryFailMessages = messages.getFailMessages()
          const successMessages = messages.getSuccessMessages()

          emitter.emit("bulk:try:success", successMessages)
          emitter.emit("bulk:try:fail", tryFailMessages)
          emitter.emit("bulk:try:end", messages)
        })

        emitter.on("bulk:try:fail", messages => {
          const messagesToRetry = messages.filter(msg => msg.count() < max_try)

          emitter.emit("bulk:try:retry", messagesToRetry)

          const messagesToCancel =  messages.filter(msg => msg.count() >= max_try)

          emitter.emit("bulk:try:cancel", messagesToCancel)
        })

        emitter.on("bulk:try:success", messages => {

          if (successCallback && messages.size() > 0)
            successCallback(messages)
              .then(res => log("debug", messages, "success callback", res))
              .catch(err => log("error", messages, "error callback", err))

        })

        emitter.on("bulk:try:cancel", messages => {

          if (failCallback && messages.size() > 0)
            failCallback(messages)
              .then(res => log("debug", messages, "failCallback success", res))
              .catch(err => log("error", messages, "failCallback success", err))
        })

        emitter.on("bulk:try:retry", messages =>

          messages.map(msg => _broker.requeue(msg))
        )

        emitter.on("bulk:try:end", messages =>
          messages.map(msg => _broker.remove(msg))
        )

        _broker.consume(msg =>
          emitter.emit("msg:received", msg)
        )
      },

      start() {

        const { log } = this

        emitter.on("msg:received", msg => {
          if (msg === null) {
            log("debug", {}, "cancelled")
            return
          }

          const doc = msg.parsedContent()

          try {

            const exec = callback(doc)

            if (exec.then)
              exec.then(ok =>
                ok && emitter.emit("msg:processed", msg)
              ).catch(err =>
                emitter.emit("msg:error", err, msg)
              )
            
          } catch (err) {
  
            emitter.emit("msg:error", err, msg)
          }
        })

        emitter.on("msg:processed", msg => {
          emitter.emit("msg:remove", msg)

          const doc = msg.parsedContent()

          if (successCallback) {
            successCallback(doc, msg)
              .then(res => log("debug", msg, "success callback", res))
              .catch(err => log("error", msg, "error callback", err))
          }
        })

        emitter.on("msg:error", (err, msg) => {
          
          log("error", msg, "try fail", err)

          if (msg.count() < max_try) {

            /* smoth the retry process */
            wait(retry_timeout)
                .catch(err2 => log("error", msg, "fail retry timeout", err2))
                .then(() => emitter.emit("msg:retry", msg))
          } else {
            emitter.emit("msg:fail", err, msg)
          }
        })

        emitter.on("msg:remove", msg => {
          _broker.remove(msg)
        })

        emitter.on("msg:retry", msg => {
          emitter.emit("msg:remove", msg)

          _broker.requeue(msg)
        })

        emitter.on("msg:fail", (err, msg) => {

          emitter.emit("msg:remove", msg)

          const doc = msg.parsedContent()

          if (failCallback) {
            failCallback(doc, err, msg)
              .then(res => log("debug", msg, "failCallback success", res))
              .catch(err => log("error", msg, "failCallback success", err))
          }
        })

        _broker.consume(msg => {
          emitter.emit("msg:received", msg)
        })


      }// end start
    }

    return {
      worker,
      publish: doc => _broker.publish(doc, uuidv4(), 1)
    }
  }
}

module.exports = WorkerFactory
