    ...
    const worker = {
      ...
      start() {
        ...
        emitter.on("bulk:try:fail", messages => {
          const messagesToRetry = messages.filter(msg => msg.count() < max_try && msg.isContinueOnError())

          emitter.emit("bulk:try:retry", messagesToRetry)

          const messagesToCancel =  messages.filter(msg => msg.count() >= max_try || !msg.isContinueOnError())
